...
parser.add_argument("--reparse", help="Parse all documents in the input directory, even those already parsed when calling this script previously.", action="store_true")
...
def main(args):
    ...
    for dirpath, dirs, files in os.walk(args.input):
        for file in files:
            ...
            if os.path.isfile(output_path) and not args.reparse:
                logger.info(f"Skip processing of {input_file}, as corresponding json output is already present in output directory.")
                logger.info("Set --reparse flag if you wish to parse all files under input directory (e.g. if they have been updated)")
            else:
                if file.startswith("."):
                    print(input_file)
